    ...
    <div className="min-h-full">
      <header className="sticky top-0 z-30 bg-cream/95 backdrop-blur-sm px-4 pt-2 pb-2 border-b border-tan/15">
        ...
        <div className="relative mb-2.5">
          ...
          <input
            ...
            className="w-full bg-paper border border-tan/30 rounded-full pl-9 pr-4 py-2 text-sm text-dark placeholder:text-brown/40 focus:outline-none focus:border-brown/50 transition-colors"
          />
        </div>
      ...
      <div className="grid grid-cols-3 gap-px bg-tan/10">
        {filtered.map((post) => (
          <div
            key={post.id}
            className="aspect-square relative film-overlay film-grain active:opacity-70 transition-opacity"
          >
            <div
      ...
      {filtered.length === 0 && (
        <div className="text-center py-20 px-6">
          ...
          <p className="text-sm text-brown/60">No moments found. Try a different search.</p>
        </div>
      )}
    </div>
  );
